...
namespace Hostify.Controllers
{
	...
	public class QuartoController : ControllerBase
	{
		...
		// Lista todos os quartos com informações do hotel
		[HttpGet]
		public async Task<ActionResult<IEnumerable<Quarto>>> GetQuarto()
		{
			return await _context.Quarto
				.Include(q => q.Hotel)
				.Where(q => q.QuartoDisponivel)
				.ToListAsync();
		}

		// Lista um quarto específico com informações do hotel
		[HttpGet("{id}")]
		public async Task<ActionResult<Quarto>> GetQuarto(int id)
		{
			var quarto = await _context.Quarto
				.Include(q => q.Hotel) // Inclui informações do hotel
				.FirstOrDefaultAsync(q => q.IdQuarto == id);
		...
		public async Task<IActionResult> BloquearQuarto(int id, [FromQuery] DateTime inicioReserva, [FromQuery] DateTime fimReserva)
		{
			...
			var reservaConflitante = await _context.Reserva
				.AnyAsync(r => r.Quarto.IdQuarto == id && inicioReserva < r.FimReserva && fimReserva > r.InicioReserva);
		...
		)
		{
			try
			{
				var quartosQuery = _context.Quarto
					.Include(q => q.Hotel) // Inclui informações do hotel na busca
					.AsQueryable();
